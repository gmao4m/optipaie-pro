...
{
    /// <summary>
    /// Builds <see cref="UpdateOptions"/> from configuration. The update source (version.json
    /// URL / GitHub repo) comes from App.config (no hardcoded URL); the update-metadata endpoint
    /// and the publishable key are derived from the already-configured licensing project, so
